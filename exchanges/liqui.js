/**
 * Liqui order book event emitter.
 *
...
const debug = require('debug')('cointrage:order_book:liqui');
...
const getOrderBook = (market, ticker) => new Promise((resolve, reject) => {
    ...
    request({
        ...
        if (!data) {
            return reject(`Invalid response: ${JSON.stringify(body)}`);
            // debug(`${ticker.toLowerCase()}_${market.toLowerCase()}`);
            // debug(body);
            // debug(data);
        }
...
class LiquiOrderBook extends EventEmitter {
...
module.exports = new LiquiOrderBook();